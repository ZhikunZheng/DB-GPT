...
class Excel2TableInput(BaseModel):
    """Excel to Table action input model supporting multiple tables with three fields."""
...
class Excel2TableAction(Action[Excel2TableInput]):
    """Action class for creating multiple tables and inserting data from Excel by field order."""
    ...
    async def run(
        ...
        rely_action_out: Optional[ActionOutput] = None,** kwargs,
    ) -> ActionOutput:
        """Perform the action: create multiple tables and insert data from corresponding Excel files."""
        try:
            # 转换AI消息为输入对象
            ...
            if len(param.create_sql_list) != len(param.table_names):
                raise ValueError(
                    f"Number of SQL statements ({len(param.create_sql_list)}) does not match number of table names ({len(param.table_names)})"
                )
                
            # 验证所有SQL是否为CREATE TABLE语句
            for sql in param.create_sql_list:
                if not sql.strip().upper().startswith("CREATE TABLE"):
                    raise ValueError(f"SQL must be a CREATE TABLE statement, got: {sql}")
                
        except Exception as e:
            logger.exception(f"Input conversion failed! \n {ai_message}")
            return ActionOutput(
                is_exe_success=False,
                content="Error: The answer is not in the required format (need 'create_sql_list', 'table_names' and 'thought' fields with matching lengths)",
            )
        ...
        try:
            ...
            for i in range(len(param.create_sql_list)):
                ...
                logger.info(f"Parsed table structure for {table_name}: {[f['name'] for f in table_fields]}")
                ...
                print(f"Generated INSERT SQL for {table_name}: {insert_sql[:200]}...")  # 打印前200字符
    ...
    def extract_table_fields(self, create_sql: str) -> List[Dict[str, str]]:
        ...
        # 提取表名和字段部分
        match = re.match(r'CREATE\s+TABLE\s+[`"]?(\w+)[`"]?\s*\((.*)\)', sql_clean, re.IGNORECASE)
        if not match:
        ...
        for field in fields:
            ...
            # 提取字段名和类型
            # 处理可能的反引号或引号包裹的字段名
            field_match = re.match(r'^[`"\[]?(\w+)[`"\]]?\s+(.+?)(\s+|$)', field, re.IGNORECASE)
            if field_match:
    ...
    def generate_insert_sql(
        ...
        # 拼接完整INSERT语句
        return f"INSERT INTO {table_name} ({', '.join(field_names)}) VALUES {', '.join(values_clauses)};"
