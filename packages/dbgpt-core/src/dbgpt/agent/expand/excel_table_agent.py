...
from ..core.agent import Agent, AgentContext, AgentMessage
...
class Excel2TableAgent(ConversableAgent):
    ...
    profile: ProfileConfig = ProfileConfig(
        ...
        goal=DynConfig(
            "Based on the Excel table header (Chinese/English) and sample data, complete 3 core tasks: "
            "1. Field name adaptive processing: If the Excel header is already in English (snake_case/camelCase), retain it directly without translation; if it is Chinese, convert it to standard English snake_case (e.g., 产品ID→product_id); "
            "2. Field order strict alignment: The field order in the CREATE TABLE SQL must be exactly the same as the header order in the Excel table (to support subsequent data insertion by field order); "
            "3. Generate {{dialect}} database table creation SQL (including primary key, field type, length constraint) and a semantic table name (snake_case). "
            "Finally, provide the result in the specified JSON format (only table name and create SQL) to support subsequent automatic data insertion.",
            category="agent",
        ...
        constraints=DynConfig(
            [
            "Field naming: Detect English vs Chinese headers.  Keep English headers in snake_case;  translate Chinese headers to meaningful English snake_case.  Ensure uniqueness.",
            "Field order: SQL field order must strictly follow Excel header order, no rearrangement.",
            "Field completeness: All Excel headers must be included in the CREATE TABLE SQL, no omission.",
            "SQL generation: Infer field types from sample data;  specify VARCHAR length;  add PRIMARY KEY for ID fields;  apply NOT NULL where required.",
            "Table naming: Use [module]_[data_type], snake_case, ≤30 chars, avoid reserved words.",
            "History consistency: If headers were converted before, keep the same English names and order;  avoid duplicates."
            ],
            category="agent",
        ...
        desc=DynConfig(
            "Store the data in one or more Excel tables mentioned by the user respectively in the database to facilitate subsequent statistical analysis of the data therein.",
            category="agent",
    ...
    async def thinking(
            ...
            excel_files = find_excel_files("./examples/test_files")
            all_file_data = []
    ...
    async def correctness_check(
            ...
            """Verify whether the current execution results meet the target expectations for multiple tables."""
            ...
            try:
                ...
                if not action_out.resource_value or not database:
                    return (
                        False,
                        "Please check your answer, the data resource information is not found.",
                    )
                ...
                for table_info in tables:
                    ...
                    if not values or len(values) <= 0 or values[0][0] == 0:
                        return (
                            False,
                            f"Table {table_name} exists but contains no data. Please check the data insertion process.",
                        )
                    
                    logger.info(
                        f"Table {table_name} verification success! There are {values[0][0]} rows of data."
                    )
                
                # 所有表验证通过
                logger.info(
                    f"All {len(tables)} tables verification success!"
                )
                return True, None
                ...
                return (
                    False,
                    f"Verification error, please re-read the historical information to fix this. "
